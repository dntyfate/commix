...
"""
About: Base64 all characters in a given payload and wrap it into
'bash<<<$(base64 -d<<<ENCODED)' so it self-decodes at runtime.
Notes: This tamper script works against all targets.
"""

__tamper__ = "base64encodewrapper"
...
def tamper(payload):
  if len(settings.WHITESPACES) != 0 and settings.WHITESPACES[0] == _urllib.parse.quote_plus(settings.SINGLE_WHITESPACE):
    ...
    wrapped = f"bash<<<$(base64 -d<<<{payload})"
    return wrapped
